...
export async function scrape4() {
  ...
  const operationOption = await selectOperation.$('.multiselectcustom__content-wrapper li#null-10');  //null8 equals
  if (operationOption) {
  ...
  const divtab = await selectOffset.$('.multiselectcustom__content-wrapper li#null-34');

 if (divtab) {
    page.once('dialog', async dialog => {
    console.log(`Dialog message: ${dialog.message()}`);
    await dialog.accept('4');
    console.log('Entered "4" in the prompt and accepted');
  });
   await delay(5000)
  ...
  await delay(5000); 
  await renameCsvFile('Stock Screener, Technical Analysis Scanner.csv', '4months.csv');
  const sourcePath = path.join(__dirname, '4months.csv');
  const destinationPath = path.join(__dirname, '4month.csv');
  copyCsv(sourcePath, destinationPath);
  const csvPath = path.join(__dirname, '4months.csv');
  deleteCsv(csvPath);
