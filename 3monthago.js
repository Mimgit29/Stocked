...
export async function scrape3() {
 ...
 if (divtab) {
    ...
    console.log('Entered "3" in the prompt and accepted');
  });
   await delay(5000)
  ...
  await renameCsvFile('Stock Screener, Technical Analysis Scanner.csv', '3months.csv');
  const sourcePath = path.join(__dirname, '3months.csv');
  const destinationPath = path.join(__dirname, '3month.csv');
  copyCsv(sourcePath, destinationPath);
  const csvPath = path.join(__dirname, '3months.csv');
  deleteCsv(csvPath);
  await browser.close();
};
