...
export async function scrape2() {
 ...
 if (divtab) {
    ...
    console.log('Entered "2" in the prompt and accepted');
  });
   await delay(5000)
  ...
  await renameCsvFile('Stock Screener, Technical Analysis Scanner.csv', '2months.csv');
  const sourcePath = path.join(__dirname, '2months.csv');
  const destinationPath = path.join(__dirname, '2month.csv');
  copyCsv(sourcePath, destinationPath);
  const csvPath = path.join(__dirname, '2months.csv');
  deleteCsv(csvPath);
  await browser.close();
};
