...
export async function scrape1() {
 ...
 if (divtab) {
    ...
    console.log('Entered "1" in the prompt and accepted');
  });
   await delay(5000)
  ...
  console.log(`File should be downloaded to: ${downloadPath}`);
   await renameCsvFile('Stock Screener, Technical Analysis Scanner.csv', '1months.csv');
  const sourcePath = path.join(__dirname, '1months.csv');
  const destinationPath = path.join(__dirname, '1month.csv');
  copyCsv(sourcePath, destinationPath);
  const csvPath = path.join(__dirname, '1months.csv');
  deleteCsv(csvPath);
  await browser.close();
};
