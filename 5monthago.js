...
export async function scrape5() {
 ...
 if (divtab) {
    ...
    console.log('Entered "5" in the prompt and accepted');
  });
   await delay(5000)
  ...
  await renameCsvFile('Stock Screener, Technical Analysis Scanner.csv', '5months.csv');
  const sourcePath = path.join(__dirname, '5months.csv');
  const destinationPath = path.join(__dirname, '5month.csv');
  copyCsv(sourcePath, destinationPath);
  const csvPath = path.join(__dirname, '5months.csv');
  deleteCsv(csvPath);
