...
export async function scrape6()  {
 ...
 if (divtab) {
    ...
    console.log('Entered "6" in the prompt and accepted');
  });
   await delay(5000)
  ...
  await renameCsvFile('Stock Screener, Technical Analysis Scanner.csv', '6months.csv');
  const sourcePath = path.join(__dirname, '6months.csv');
  const destinationPath = path.join(__dirname, '6month.csv');
  copyCsv(sourcePath, destinationPath);
  const csvPath = path.join(__dirname, '6months.csv');
  deleteCsv(csvPath);
  await browser.close();
};
